...
- SELECT first_name, last_name, count(*) AS count FROM owners JOIN vehicles ON vehicles.owner_id = owners.id GROUP BY first_name, last_name ORDER BY count;
...
SELECT first_name, last_name,AVG (ROUND(price)) AS average_price, count(*) AS count FROM owners JOIN vehicles ON vehicles.owner_id = owners.id GROUP BY first_name, last_name HAVING count(*) > 1 AND AVG(price) > 10000 ORDER BY first_name desc;